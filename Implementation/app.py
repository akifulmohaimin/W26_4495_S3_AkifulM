...
# If your Windows PATH for Tesseract is not working, set it directly:
# pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"

# If pdf2image needs Poppler on Windows, set this:
...
def extract_embedded_pdf_text(pdf_path: Path) -> str:
    ...
    If the PDF is scanned (no embedded text), this may return almost nothing.
    """
...
def extract_report_text(file_path: str, poppler_path: Optional[str] = None) -> Dict[str, Any]:
    """
    Returns a record dict containing:
...
def parse_reference_range(ref: str) -> Tuple[Optional[float], Optional[float]]:
    """
    Supports reference formats like:
    - "13 - 17"
    - "31.5 – 34.5"
    - "< 2"
    """
...
def extract_cbc(text: str) -> Dict[str, Dict[str, Any]]:
    """
    Extract CBC fields from the raw extracted text using regex patterns.
    Each extracted indicator is returned with reference values + a flag.
    """
    results = {}
    ...
    for indicator, pattern in CBC_PATTERNS.items():
        ...
        # HCT is a special case because the capture groups differ
        if indicator == "HCT %":
            ...
            # Hemoglobin pattern stores ref range in group(4); most others in group(3)
...
if __name__ == "__main__":
    ...
    record = extract_report_text(sample_file, poppler_path=POPPLER_PATH)

    print("Status:", record["status"])
    print("Patient ID:", record["patient_id"])
    print("Extracted chars:", len(record.get("raw_text", "")))

    if record["status"] == "Failed":
        print("ERROR:", record.get("error"))
        raise SystemExit(1)

    # Stage 2: structure CBC indicators
    record["processed_indicators"]["cbc"] = extract_cbc(record["raw_text"])
    record["status"] = "Structured"

    print("\nCBC extracted fields:", len(record["processed_indicators"]["cbc"]))
    for name, info in record["processed_indicators"]["cbc"].items():
        unit = info.get("unit") or ""
        print(f"{name}: {info['value']} {unit} => {info['flag']}")

    # Save output
    saved_path = save_json_record(record)
    print("\nSaved structured record to:", saved_path)